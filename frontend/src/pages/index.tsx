...
import { MediaType } from '@/components/chat/MediaUpload'
import { SocialDeck } from '@/components/social/SocialDeck'
...
export default function NexusApp() {
  const { setUser, setToken, setStatus, status, user, token, partnerInfo, wsStatus } = useNexusStore()
  const { isOnboarded, profile } = useUserStore()
  const { theme } = useTheme()
  const [mobileTab, setMobileTab] = useState<'video' | 'social' | 'chat'>('video')
  ...
  useEffect(() => {
    ...
    if (!window.isSecureContext && !isLocalhost) {
      window.location.replace('https://vox-bridge-ivory.vercel.app')
    }
  }, [])
  ...
  useEffect(() => {
    if (mobileTab === 'social' || desktopSocialOpen) {
      wsRef.current.send('get_discovery')
      wsRef.current.send('get_friends')
    }
  }, [mobileTab, desktopSocialOpen])
  ...
  const handleSendMedia = (type: MediaType, data: string, fileName: string, clientId: string) => {
    return wsRef.current.send('media_message', { type, data, fileName, clientId })
  }
  ...
  return (
    <NexusGrid activeTab={mobileTab} onTabChange={setMobileTab}>
      ...
      <Column width="flex-1" className={`relative ${mobileTab !== 'video' ? 'hidden md:block' : ''}`}>
        ...
        <button
          onClick={() => setDesktopSocialOpen(true)}
          className="hidden md:flex absolute left-5 bottom-5 z-40 h-11 items-center gap-2 rounded-xl border border-white/10 bg-black/70 px-4 text-sm font-bold text-white shadow-xl backdrop-blur-xl transition hover:bg-white/15"
        >
          <svg className="h-4 w-4 text-cyan-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-6.63a4 4 0 11-8 0 4 4 0 018 0zm8 2a3 3 0 11-6 0 3 3 0 016 0z" />
          </svg>
          Social
        </button>

        {desktopSocialOpen && (
          <div className="hidden md:block absolute inset-4 z-50 overflow-hidden rounded-2xl border border-white/10 bg-[#08090b] shadow-2xl shadow-black/60">
            <button
              onClick={() => setDesktopSocialOpen(false)}
              className="absolute right-4 top-4 z-[60] flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-black/55 text-white backdrop-blur-xl transition hover:bg-white/10"
              title="Fechar social"
            >
            ...
            <SocialDeck
              onStartMatch={status === 'connected' ? handleNext : handleStartMatch}
              onOpenChat={() => setRightSidebarOpen(true)}
              onRefresh={ws.getDiscovery}
              onAddFriend={ws.sendFriendRequest}
              onRemoveFriend={ws.removeFriend}
            />
          </div>
        )}
          ...
          <div className="absolute inset-0 z-20 flex items-center justify-center p-4">
            ...
            <div className="relative text-center">
              <button 
                ...
                className="group relative px-10 py-5 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-bold rounded-2xl shadow-2xl shadow-cyan-500/30 hover:shadow-cyan-500/50 hover:scale-105 active:scale-95 transition-all duration-300 disabled:cursor-wait disabled:opacity-60 disabled:hover:scale-100"
              >
                <span className="relative z-10 flex items-center gap-3 text-lg">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                  {wsStatus === 'connecting' || wsStatus === 'reconnecting' ? 'Conectando...' : 'Iniciar'}
                </span>
              </button>
      ...
      {/* Social Layer */}
      <div
        className={`${mobileTab !== 'social' ? 'hidden' : 'fixed inset-0 z-40 pb-16 md:hidden'} transition-all duration-300`}
      ...
      <button 
        ...
        style={{ 
          right: rightSidebarOpen ? '384px' : '0',
          background: 'rgba(0,0,0,0.5)',
          backdropFilter: 'blur(8px)'
        }}
      >
      ...
      {/* Right Sidebar - Chat */}
      <div className={`${mobileTab !== 'chat' ? 'hidden md:block' : 'fixed inset-0 z-40 pb-16 md:relative md:inset-auto md:pb-0 md:z-auto'} transition-all duration-300 ${rightSidebarOpen ? 'md:w-96' : 'md:w-0'} md:overflow-hidden`}
